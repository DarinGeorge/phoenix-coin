export const styles = {
  container: 'flex-col items-center justify-center px-5 mb-5 border-t pt-3',
  title: 'font-medium w-full text-center',
  input: 'w-12 text-center border-none text-2xl',
  amount: 'font-medium text-2xl mr-1 text-center ml-3',
  indicator: 'p-2 text-2xl font-bold',
  actionBtnWrapper: 'flex flex-row items-center justify-center mb-2',
  button: 'w-full bg-transparent text-md rounded-md border-2 border-gray-200 py-1 px-3 font-semibold text-purple-600',
};
